...
public class Setup {
	...
	public Setup(WebDriver d, String tcID) {
		this.driver = d;
		this.TC_ID = tcID;
	}
	...
	{		
		...
		String log = currentDir+ "\\bulk-op.txt";
		String mg = currentDir+ "\\mg_op.txt";
		FileChannel.open(Paths.get(log), StandardOpenOption.WRITE).truncate(0).close();
		FileChannel.open(Paths.get(mg), StandardOpenOption.WRITE).truncate(0).close();
		...
		//Checking and setting the region
		String rg = Excel_Handling.Get_Data(TC_ID, "region");
		method.set_region(driver, "csdx config:set:region ", rg, "https://stag-api.contentstack.io ", "https://stag-cdn.contentstack.io ", "Custom region has been set to", "Region is set");
